...
```solidity
function permit(address owner, address spender, uint256 value, uint256 deadline, bytes sig) external
```
...
```solidity
function claimFeesView(address recipient) external view returns (uint256 claimed0, uint256 claimed1)
```
